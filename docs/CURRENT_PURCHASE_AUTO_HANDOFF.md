...
- `/app/approval/document/new`가 `결재문서를 열람할 수 없습니다` 오류 모달을 띄우면 닫고, `새 결재`/`기안하기`/`결재 작성` 메뉴와 `/app/approval/document/new/223` 후보 URL을 순서대로 시도한다.
- 양식 클릭이 새 창/탭으로 열리는 경우도 감지해서 새 페이지를 반환한다.
