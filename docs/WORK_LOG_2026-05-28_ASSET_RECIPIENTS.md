...
- 집기비품이 포함된 구매에서 지급대상이 품목별로 입력되지 않고 전역 기본값으로 뭉개졌다.
  - 집기비품별로 부서, 사용자, 용도를 구매 단계에서 받아야 한다.
...
- `PurchaseItem`에 항목별 지급 정보 필드를 추가했다.
  ...
  - `asset_purpose`
  - `asset_note`
- 집기비품/소프트웨어 품목인데 지급 정보가 없으면 품의 본문 생성 전에 오류를 낸다.
- 품목/모델 분리 규칙을 보강했다.
...
& "C:\Users\user\AppData\Local\Programs\Python\Python311\python.exe" -m py_compile purchase_auto\models.py purchase_auto\compuzone_order.py purchase_auto\groupware_approval.py tests\test_state_flow.py
node --check "C:\Users\user\Desktop\개발파일\DS_WMS\backend\routes\purchaseCart.js"
cd "C:\Users\user\Desktop\개발파일\DS_WMS\frontend"
npm.cmd run build
```

- Purchase_Auto pytest: 32 passed
